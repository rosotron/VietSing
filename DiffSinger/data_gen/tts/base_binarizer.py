...
class BaseBinarizer:
    ...
    def build_spk_map(self):
        ...
        for item_name in self.item_names:
            # based on the str before '-' of subfolders
    ...
    def meta_data(self, prefix):
        if prefix == 'valid':
            ...
            print('valid len', len(item_names))
        elif prefix == 'test':
            item_names = self.test_item_names
            print('test len', len(item_names))
        else:
            item_names = self.train_item_names
            print('train len', len(item_names))
    ...
    def process_data(self, prefix):
        data_dir = hparams['binary_data_dir'] # data/binary/popcs-pmf0 # data/binary/opencpop-midi-dp
        args = []
        builder = IndexedDatasetBuilder(f'{data_dir}/{prefix}') # Outputs the .data and .idx for train/valid/test
        lengths = []
        f0s = []
        total_sec = 0
        if self.binarization_args['with_spk_embed']: # False (SINGLE) # True (MULTISPEAKER)
            voice_encoder = VoiceEncoder().cuda()

        meta_data = list(self.meta_data(prefix)) # return generator object of item_name, ph, txt, tg_fn, wav_fn, spk_id
        for m in meta_data:
        ...
        for f_id, (_, item) in enumerate(
            ...
            # f0 extracted during self.process_item
            # print(item)
            # item: dict
            # item_name: str --> 'popcs-Bad-0000', txt: str --> lyrics, ph: str --> _ph.txt, 
            # mel: 2D nparray, wav: 1D nparray, wav_fn: str --> _wf0.wav directory
            # sec: float --> total duration, len: int --> num of samples, spk_id: int --> 0
            # (MIDI) pitch_midi: 1D nparray --> encoded note, (MIDI) midi_dur: 1D nparray --> duration of each note
            # (MIDI) is_slur: 1D nparray: 1 if corresponding phn is slur, (MIDI) word_boundary: 0 if start of word
            # f0: 1D nparray, pitch: 1D nparray, phone: list --> one-hot encoded phones
            # mel2ph: 1D nparray --> mel alignment with ph, (GT) dur: 1D nparray --> num of samples per ph, sums to len 
            if item is None:
                continue
            item['spk_embed'] = voice_encoder.embed_utterance(item['wav']) \
                if self.binarization_args['with_spk_embed'] else None
            # spk_embed: 1D nparray from embedding wav

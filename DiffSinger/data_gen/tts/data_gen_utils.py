...
def get_mel2ph(tg_fn, ph, mel, hparams):
    ph_list = ph.split(" ") # phonemes are separated by ' ' in popcs
    ...
    # split is a nparray to contain the starting time of phonemes
    tg_idx = 0
    ph_idx = 0
    ...
    for x in tg_align:
        x['xmin'] = float(x['xmin'])
        x['xmax'] = float(x['xmax'])
        ''' Treat 'spn' as a silent phoneme. '''
        # Replace 'sil', 'sp', '', 'SIL', 'PUNC', 'spn' as ''
        if x['text'] in ['sil', 'sp', '', 'SIL', 'PUNC', 'ap']: # spn is a stand-in fake phoneme to mean a spoken sound/noise.
        # if x['text'] in ['sil', 'sp', '', 'SIL', 'PUNC']:
            ...
            if len(tg_align_) > 0 and tg_align_[-1]['text'] == '':
                ...
                continue
        tg_align_.append(x)
    tg_align = tg_align_ # len(tg_align) == num of intervals in phones tier
    tg_len = len([x for x in tg_align if x['text'] != ''])
    ph_len = len([x for x in ph_list if not is_sil_phoneme(x)]) 
    # 'sil', 'sp', '', 'SIL', 'PUNC', 'spn' needs to be isalpha() false.
    # print(tg_len) 
    # print(ph_len)
    # print([x for x in tg_align if x['text'] != ''])
    # print([x for x in ph_list if not is_sil_phoneme(x)])
    assert tg_len == ph_len, (tg_len, ph_len, tg_align, ph_list, tg_fn)
    # print(len(tg_align))
    # print(len(ph_list)) # number of phonemes including 'sil', 'sp', '', 'SIL', 'PUNC', 'spn'
    # print(tg_align)
    # print(ph_list)
    # print(tg_fn)
    while tg_idx < len(tg_align) or ph_idx < len(ph_list):
        # last item of split is 1e8
    ...
    for i in range(len(split) - 1):
        if not (split[i] != -1 and split[i] <= split[i + 1]):
            print(i)
            print(tg_fn)
        assert split[i] != -1 and split[i] <= split[i + 1], (split[:-1],)
    # with the starting time of phonemes we then calculate the starting sample of each phoneme 
    split = [int(s * hparams['audio_sample_rate'] / hparams['hop_size'] + 0.5) for s in split]
    # print(split)
    # print(len(mel2ph))
    # mel2ph: index is the mel and value is corresponding index of the phoneme in ph

...
from data_gen.tts.txt_processors.zh_g2pM import ALL_YUNMU, M4SINGER_YUNMU
...
class SingingBinarizer(BaseBinarizer):
    ...
    def load_meta_data(self):
        for ds_id, processed_data_dir in enumerate(self.processed_data_dirs):
            # print(ds_id) # 0
            # print(processed_data_dir) # data/processed/popcs
            wav_suffix = '_wf0.wav'
            txt_suffix = '.txt'
            ph_suffix = '_ph.txt'
            tg_suffix = '.TextGrid'
            ...
            # list of all wav files from all subfolders in data/processed/popcs 

            for piece_path in all_wav_pieces:
                # subfolder name + '-' + piece index w/o file extension
                ...
                # Spk ID is the str before '-' in subfolders
        ...
        self.item_names = sorted(list(self.item2txt.keys())) # sorted list of item names
        if self.binarization_args['shuffle']: # set to false
            random.seed(1234)
    ...
    def _phone_encoder(self):
        ...
        ph_set = []
        # either reset_phone_dict: True or phone_set.json does not exist
        # then the json file is outputted
        if hparams['reset_phone_dict'] or not os.path.exists(ph_set_fn):
            for ph_sent in self.item2ph.values():
                ph_set += ph_sent.split(' ') # each phoneme is separated by ' ' for popcs and Opencpop # CHANGE ACCORDINGLY
...
class MidiSingingBinarizer(SingingBinarizer):
    ...
    def load_meta_data(self):
        ...
        if self.binarization_args['shuffle']:
            random.seed(1234)
            random.shuffle(self.item_names)
        # based on your test_prefixes param in usr/popcs_ds_beta6.yaml
        self._train_item_names, self._test_item_names = self.split_train_test_set(self.item_names)
...
class VietSingBinarizer(MidiSingingBinarizer):
    item2midi = {}
    item2midi_dur = {}
    item2is_slur = {}
    item2ph_durs = {}
    ...
    def load_meta_data(self):
        ...
        utterance_labels = os.listdir(os.path.join(raw_data_dir, 'wavs/'))
        for utterance_label in utterance_labels:
            item_name = raw_item_name = utterance_label.replace('.wav', '') # 20010001
            self.item2wavfn[item_name] = os.path.join(raw_data_dir, 'wavs', utterance_label) # 20010001.wav
            self.item2txt[item_name] = open(os.path.join(raw_data_dir, 'word', item_name+'.txt')).readline() # words
            self.item2spk[item_name] = 'vietsing'
        
            with open(os.path.join(raw_data_dir, 'transcriptions.txt'), 'r') as tr:
                lines = tr.readlines()
                for line in lines:
                    song_info = line.split('|')
                    if song_info[0] == item_name:
                        break
                self.item2ph[item_name] = song_info[2] # phones
                self.item2wdb[item_name] = [int(x) for x in song_info[7].split(" ")] #word boundary
                self.item2ph_durs[item_name] = [float(x) for x in song_info[5].split(" ")] #phonemes durations

                self.item2midi[item_name] = [librosa.note_to_midi(x) if x != 'rest' else 0
                ...
                assert len(song_info[2].split(" "))==len(song_info[3].split(" "))==len(song_info[4].split(" "))==len(song_info[5].split(" "))==len(song_info[6].split(" "))==len(song_info[7].split(" "))
    ...
    def get_pitch(wav_fn, wav, spec, ph, res):
        ...
        res['pitch_midi'] = np.asarray(VietSingBinarizer.item2midi[item_name])
        res['midi_dur'] = np.asarray(VietSingBinarizer.item2midi_dur[item_name])
        res['is_slur'] = np.asarray(VietSingBinarizer.item2is_slur[item_name])
        res['word_boundary'] = np.asarray(VietSingBinarizer.item2wdb[item_name])
    ...
    @classmethod
    def process_item(cls, item_name, ph, txt, tg_fn, wav_fn, spk_id, encoder, binarization_args):
        if hparams['vocoder'] in VOCODERS: # VOCODERS = {}
            wav, mel = VOCODERS[hparams['vocoder']].wav2spec(wav_fn)
        else: # vocoders.hifigan.HifiGAN
            wav, mel = VOCODERS[hparams['vocoder'].split('.')[-1]].wav2spec(wav_fn) # HiFiGAN is child class of PWG, wav2spec by PWG
        res = {
            'item_name': item_name, 'txt': txt, 'ph': ph, 'mel': mel, 'wav': wav, 'wav_fn': wav_fn,
            'sec': len(wav) / hparams['audio_sample_rate'], 'len': mel.shape[0], 'spk_id': spk_id
        }
        try:
            if binarization_args['with_f0']: # True
                cls.get_pitch(wav_fn, wav, mel, ph, res)
            if binarization_args['with_txt']: # True
                try:
                    phone_encoded = res['phone'] = encoder.encode(ph)
                except:
                    traceback.print_exc()
                    raise BinarizationError(f"Empty phoneme")
                if binarization_args['with_align']: # True
                    cls.get_align(VietSingBinarizer.item2ph_durs[item_name], mel, phone_encoded, res)
        except BinarizationError as e:
            print(f"| Skip item ({e}). item_name: {item_name}, wav_fn: {wav_fn}")
            return None
        return res

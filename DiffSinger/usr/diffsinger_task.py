...
class VietSingDataset(FastSpeechDataset):
    def __getitem__(self, index):
        sample = super(VietSingDataset, self).__getitem__(index)
        item = self._get_item(index)
        sample['pitch_midi'] = torch.LongTensor(item['pitch_midi'])[:hparams['max_frames']]
        sample['midi_dur'] = torch.FloatTensor(item['midi_dur'])[:hparams['max_frames']]
        sample['is_slur'] = torch.LongTensor(item['is_slur'])[:hparams['max_frames']]
    ...
    def collater(self, samples):
        batch = super(VietSingDataset, self).collater(samples)
        batch['pitch_midi'] = utils.collate_1d([s['pitch_midi'] for s in samples], 0)
        batch['midi_dur'] = utils.collate_1d([s['midi_dur'] for s in samples], 0)
        batch['is_slur'] = utils.collate_1d([s['is_slur'] for s in samples], 0)
        batch['word_boundary'] = utils.collate_1d([s['word_boundary'] for s in samples], 0)
...
class DiffSingerMIDITask(DiffSingerTask):
    def __init__(self):
        ...
        self.dataset_cls = VietSingDataset

    def run_model(self, model, sample, return_output=False, infer=False):
        # B --> batch size, T_s --> num of samples
        txt_tokens = sample['txt_tokens']  # [B, T_t] # phone
        # print(txt_tokens[0])
        target = sample['mels']  # [B, T_s, 80]
        # print(target[0])
    ...
    def validation_step(self, sample, batch_idx):
        ...
        outputs['nsamples'] = sample['nsamples']
        outputs = utils.tensors_to_scalars(outputs)
        if (hparams['num_spk'] == 1 and batch_idx < hparams['num_valid_plots']) or (batch_idx % hparams['num_spk'] == 0 and batch_idx // hparams['num_spk'] < hparams['num_valid_plots']):
            model_out = self.model(
    ...
    def add_dur_loss(self, dur_pred, mel2ph, txt_tokens, wdb, losses=None):
        ...
        if hparams['lambda_word_dur'] > 0:
            # print(wdb)
            # print(wdb.size())
            ### If using Opencpop/M4Singer with default wdb ###
            idx = F.pad(wdb.cumsum(axis=1), (1, 0))[:, :-1] # Pads a 0 at the start and removes the last element from tensor 
            ### If using self crafted wdb/other datasets ###
            # idx = wdb.cumsum(axis=1) 

            # idx = torch.sub(idx, 1) # Commented out in case wdb beginned with 0 (Should not be but just in case) 
            # print(idx[0])
            # print(idx[0].size())
            # print(dur_pred[0])
            # word_dur_g = dur_gt.new_zeros([B, idx.max() + 1]).scatter_(1, idx, midi_dur)  # midi_dur can be implied by add gt-ph_dur
            word_dur_p = dur_pred.new_zeros([B, idx.max() + 1]).scatter_add(1, idx, dur_pred) # Adds the values of tensor A based on a idx tensor B
            # print(word_dur_p[0])
